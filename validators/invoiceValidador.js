...
const invoiceValidationRules = [
    body("fiscalCode")
        ...
        .isNumeric().withMessage("O código fiscal deve conter apenas números.") // Substitui .isString() e valida "apenas número"
        .isLength({ min: 44, max: 44 }).withMessage("O código fiscal deve ter 44 caracteres.") // Adicionei um min: 7 com base na necessidade de ter o YYMM + dígito verificador. Ajuste se houver um tamanho fixo.
        ...
        .custom((value, { req }) => {
            ...
            if (isNaN(dateInvoice)) { // Garante que a conversão para número foi bem-sucedida
        ...
        .custom(async (value, { req }) => {
            ...
            if (id) { // Se for uma atualização (ID presente na URL)
                sql += ' AND id != $2';
                params.push(id);
            }
    ...
    // A validação de client_id permanece a mesma
    body("clientId")
